...
class DataLoaderForTableQA(DataLoaderWrapper):
                ...
                if "use_original_tapas_data" in module_config.config.preprocess:
                    ...
                    def attach_tf_input_to_dataset(example):
                        ...
                        if tf_input_example is not None:
                            ...
                            example["aggregation_function"] = tf_input_example[
                    ...
                    conversion_success = len(
                        dataset.filter(lambda x: x["valid"] == True)
                    )
                    conversion_error = len(
                        dataset.filter(lambda x: x["valid"] == False)
                    )
                    ...
                    def split_table_by_column_combination(example):
                        ...
                        for max_column in range(1, MAX_COLUMNS + 1):
                            for comb in combinations(
                                ...
                                selected_rows = [
                                    [row[i] for i in comb] for row in table["rows"]
                                ]
                                ...
                                sub_table["id"] = (
                                    f"{table['id']}_COLUMN({','.join(str_comb)})"
                                )
                        ...
                        example["num_sub_tables"] = len(positive_subtables) + len(
                            negative_subtables
                        )
                        return example
        ...
        for split, split_file_path in module_config.config.path.items():
            ...
            if os.path.exists(split_file_path) and module_config.option == "default":
                ...
                def preprocess_tableqa_function(example):
                    ...
                    answer_list, answer_coordinates, aggregation_op, conditions = (
                        retrieve_wikisql_query_answer_tapas(tapas_table, example_sql)
                    )
                ...
                dataset = dataset.add_column(
                    "question_id",
                    list(
                        range(current_question_id, current_question_id + len(dataset))
                    ),
                )
                ...
                if "move_answers_to_table_end" in module_config.config.preprocess:
                    ...
                    def move_answers_to_table_end(example):
                        ...
                        gold_col = [coord[1] for coord in answer_coordinates] + [
                            example["sql"]["sel"]
                        ]
                        ...
                        column_mapping = {
                            f: t
                            for f, t in zip(shifted_column_ids, original_column_ids)
                        }
                        row_mapping = {
                            f: t for f, t in zip(shifted_row_ids, original_row_ids)
                        }
                        ...
                        new_example_sql["conds"]["column_index"] = [
                            column_mapping[f]
                            for f in new_example_sql["conds"]["column_index"]
                        ]
                        new_example_sql["sel"] = column_mapping[new_example_sql["sel"]]

                        # change other content
                        example["conditioned_columns_origin"] = example[
                            "conditioned_columns"
                        ]
                        example["conditioned_columns"] = [
                            column_mapping[i] for i in example["conditioned_columns"]
                        ]
                        example["answer_coordinates_origin"] = example[
                            "answer_coordinates"
                        ]
                        example["answer_coordinates"] = [
                            [row_mapping[r], column_mapping[c]]
                    ...
                    def split_table_by_column(example):
                        ...
                        for i in range(num_columns):
                            for j in range(i + 1, num_columns):
                                ...
                                selected_rows = [
                                    [row[i], row[j]] for row in table["rows"]
                                ]
                                ...
                                sub_table["rows"] = selected_rows
                                sub_table["types"] = selected_types
                                sub_table["is_gold"] = (i in gold_col) or (
                                    j in gold_col
                                )
                ...
                if (
                    "remove_no_pos_samples_for_training"
                    in module_config.config.preprocess
                ):
                    ...
                    for line in tqdm(json_file):
                        ...
                        for question_dict in line_data["questions"]:
                            ...
                            for alternative_answer in question_dict.get(
                                ...
                                if (
                                    alternative_answer == answers
                                    or alternative_answer in alternative_answer
                                ):
                                    continue
    ...
    def LoadE2EWTQData(self, module_config):
        ...
        table_cache_path = os.path.join(
            self.config.DATA_FOLDER, module_config.config.path.tables
        )

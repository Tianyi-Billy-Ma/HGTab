...
def _find_matching_coordinates(table, answer_text, normalize):
...
def _compute_cost_matrix_inner(
    ...
    for index, answer_text in enumerate(answer_texts):
        found = 0
        for row, column in _find_matching_coordinates(table, answer_text, normalize):
            found += 1
...
def _parse_answer_coordinates(table, answer_texts, discard_ambiguous_examples):
    ...
    if cost_matrix is None:
        return
    row_indices, column_indices = scipy.optimize.linear_sum_assignment(cost_matrix)

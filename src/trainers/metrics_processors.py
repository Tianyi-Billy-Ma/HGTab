...
class MetricsProcessor:
    ...
    def compute_tapas_denotation_accuracy(self, module, data_dict, log_dict):
        ...
        def get_denotation_accuracy_with_original_table_overflow(
            ...
            for (
                ...
                raw_cell_values,
            ) in zip(predictions, references, pred_aggregation, overflows, raw_cells):
                if agg_str == "NONE":
                    # perform denotation as usual
                    is_correct = evaluate_example(
                        predict_str.lower(), ground_str.lower()
            ...
            return (
                overflow_correct_num / len(predictions),
                non_overflow_correct_num / len(predictions),
                zero_div(overflow_correct_num, overflow_num),
                zero_div(non_overflow_correct_num, non_overflow_num),
            )
        ...
        if check_original_table_overflow:
            (
                overflow_accuracy_global,
                non_overflow_accuracy_global,
                overflow_accuracy,
                non_overflow_accuracy,
            ) = get_denotation_accuracy_with_original_table_overflow(
                decoded_preds,
                decoded_labels,
                pred_aggregation,
                original_table_overflows,
            ...
            log_dict.metrics[f"{prefix}global_overflow_denotation_accuracy"] = (
                overflow_accuracy_global
            )
            log_dict.metrics[f"{prefix}global_non_overflow_denotation_accuracy"] = (
                non_overflow_accuracy_global
            )
            log_dict.metrics[f"{prefix}overflow_denotation_accuracy"] = (
                overflow_accuracy
            )
            log_dict.metrics[f"{prefix}non_overflow_denotation_accuracy"] = (
                non_overflow_accuracy
            )
        # input('done')
        return log_dict
    ...
    def compute_denotation_accuracy(self, module, data_dict, log_dict):
        ...
        decoded_preds = [
            pred_dict["decoded_output"] for pred_dict in data_dict["batch_predictions"]
        ]
        decoded_labels = [
            pred_dict["decoded_label"] for pred_dict in data_dict["batch_predictions"]
        ]
        ...
        if check_original_table_overflow:
            ...
            log_dict.metrics["global_overflow_denotation_accuracy"] = (
                overflow_accuracy_global
            )
            log_dict.metrics["global_non_overflow_denotation_accuracy"] = (
                non_overflow_accuracy_global
            )
    ...
    def compute_ITR_RAG_retrieval_results(self, module, data_dict, log_dict) -> dict:
        ...
        Ks = [
            K
            for K in Ks
            if K <= self.config.data_loader.additional.num_knowledge_passages
        ]
        ...
        for retrieval_result in batch_results:
            ...
            overflow_resolve_success = (
                non_overflow_full_recall and original_table_overflow
            )
            overflow_resolve_fail = (
                not non_overflow_full_recall
            ) and original_table_overflow
        ...
        result["non_overflow_full_recall_rate"] = (
            result["non_overflow_full_recall_rate"] / count
        )
        result["overflow_resolve_success_rate"] = (
            result["overflow_resolve_success_rate"] / count
        )
        result["overflow_resolve_fail_rate"] = (
            result["overflow_resolve_fail_rate"] / count
        )
    ...
    def compute_ITR_retrieval_results(self, module, data_dict, log_dict) -> dict:
        ...
        for retrieval_result in batch_results:
            ...
            for index, K in enumerate(Ks):
                ...
                precision_at_K = sum(truncated_num_hit_list) / len(
                    truncated_num_hit_list
                )
                # full recall measures whether all gold columns are collected
                full_recall_at_K = len(retrieval_result[gold_ref]) <= sum(
                    truncated_num_hit_list
                )
    ...
    def compute_TQA_DPR_scores(self, module, data_dict, log_dict) -> dict:
        ...
        for retrieved_results in batch_results:
            ...
            hit_list = [
                retrieved_table_id in pos_item_ids
                for retrieved_table_id in retrieved_tables_sorted
            ]
            for index, K in enumerate(Ks):
    ...
    def compute_TQA_retrieval_results(self, module, data_dict, log_dict) -> dict:
        batch_results = data_dict["batch_retrieval_results"]
        Ks = self.config.model_config.Ks
        truncate = module.get("truncate", False)
        ...
        for retrieval_result in batch_results:
            ...
            retrieved_tables = retrieval_result["retrieved_tables_sorted"]
            num_hit_list = []
            for retrieved_table, rating in retrieved_tables:
                num_hit_list.append(retrieved_table["is_gold"] == True)
            for index, K in enumerate(Ks):
                # obtain the list wrt K
                truncated_num_hit_list = num_hit_list[: min(K, len(num_hit_list))]
                # compute the recall and precision

    ...
    def evaluate_outputs(
        ...
        for step_output in step_outputs:
            ...
            for question_id, sub_table_list in zip(
                ...
                question_id_to_sub_table_index[question_id] = (
                    len(sub_tables),
                    len(sub_tables) + len(sub_table_list),
                )
            ...
            for question_id, gold_columns, gold_rows in zip(
                step_output["question_ids"],
                step_output["gold_columns"],
                step_output["gold_rows"],
            ):
        ...
        for i_batch_id in tqdm(range(n_item_batchs)):
            ...
            item_input_ids, item_attention_mask = (
                item_encoding.input_ids,
                item_encoding.attention_mask,
            )
        ...
        for question_id, start_end_index in tqdm(
            ...
            query_sub_table_ratings = rate_batch[
            ...
            zipped_query_sub_tables = [
                (query_sub_table, query_sub_table_rating)
                for query_sub_table, query_sub_table_rating in zip(
                    query_sub_tables, query_sub_table_ratings
                )
            ]
